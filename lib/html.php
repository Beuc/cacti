...
function html_start_box($title, $width, $div, $cell_padding, $align, $add_url_or_buttons, $add_label = false, $showcols = false) {
	...
	if ($title != '') {
		...
		if ($help_file !== false && $help_count == 0 && is_realm_allowed(28)) {
			print "<span class='cactiHelp' title='" . __esc('Get Page Help') . "'><a class='linkOverDark helpPage' data-page='" . html_escape(basename($help_file)) . "' href='#'><i class='fa fa-question-circle actionHelp'></i></a></span>";
			$help_count++;
		}
...
function graph_drilldown_icons($local_graph_id, $type = 'graph_buttons', $tree_id = 0, $branch_id = 0) {
	...
	if (read_config_option('realtime_enabled') == 'on' && is_realm_allowed(25)) {
		if (read_user_setting('realtime_mode') == '' || read_user_setting('realtime_mode') == '1') {
			print "<a class='iconLink realtime' href='#' role='link' id='graph_" . $local_graph_id . "_realtime'><i class='drillDown fas fa-chart-area realTime' title='" . __esc('Click to view just this Graph in Real-time'). "'></i></a><br>";
		} else {
...
function html_show_tabs_left() {
	...
	foreach ($tabs_left as $tab) {
		...
		print "<li><a id='$id' role='tab' class='lefttab" . (isset($tab['selected']) ? " selected' aria-selected='true'":"' aria-selected='false'") . " href='" . html_escape($tab['url']) . "'><span class='ti glyph_$id'></span><span class='text_$id'>" . html_escape($tab['title']) . "</span></a><a id='menu-$id' class='maintabs-submenu' href='#'><i class='fa fa-chevron-down'></i></a></li>";
	}
