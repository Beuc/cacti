...
class Installer implements JsonSerializable {
	...
	public function __construct($install_params = array()) {
		...
		$this->iconClass = array(
			...
			DB_STATUS_SKIPPED => 'ti ti-check-circle'
		);
	...
	public function processStepWelcome() {
		...
		$themePath   = CACTI_PATH_INCLUDE . '/themes/';
	...
	public function processStepCheckDependencies() {
		...
		$test_config_path    = CACTI_PATH_URL . 'install/';
	...
	public function processStepBinaryLocations() {
		...
		foreach ($this->paths as $name => $array) {
			...
			print "<div class='formRow $class'><div class='formColumnLeft'><div class='formFieldName'>" . $array['friendly_name'] . "<div class='formTooltip'><div class='cactiTooltipHint ti ti-help-circle-filled'><span style='display:none;'>" . $array['description'] . '</span></div></div></div></div>';
	...
	public function processStepPermissionCheck() {
		...
		if (isset($writable)) {
			...
			if ($config['cacti_server_os'] == 'unix') {
				...
				foreach ($paths as $path => $valid) {
					if (!$valid) {
						$code = sprintf('icacls %s %s/resource/', $running_user, CACTI_PATH_BASE);
					}
				}
			}
		...
		if (cacti_sizeof($tables)) {
			...
			if ($max_vars < cacti_count($tables) + 10) {
				...
				if ($show_warning) {
					...
					$output .= Installer::sectionCode(read_config_option('path_php_binary') . ' -q ' . CACTI_PATH_CLI . '/convert_tables.php -u -i');
				}
	...
	private static function disableInvalidPlugins() {
		...
		if (cacti_sizeof($plugins)) {
			foreach ($plugins as $plugin => $version) {
				$disable    = true;
				$integrated = in_array($plugin, $plugins_integrated, true);

				set_install_config_option('install_updated', microtime(true));

				if (
					is_dir(CACTI_PATH_PLUGINS . '/' . $plugin)
					&& file_exists(CACTI_PATH_PLUGINS . "/$plugin/setup.php")
					&& file_exists(CACTI_PATH_PLUGINS . "/$plugin/INFO")
					&& !$integrated
				) {
					$info = parse_ini_file(CACTI_PATH_PLUGINS . "/$plugin/INFO", true);
